...
if [ "$stage" = smoke ]; then
    ...
    if [ -n "${SEMIGROUP_SOURCE_COMMIT:-}" ]; then
        printf '%s\n' "$SEMIGROUP_SOURCE_COMMIT" > "$run_root/source_commit.txt"
    else
...
set -eu
export OMP_NUM_THREADS=1 MKL_NUM_THREADS=1
export SEMIGROUP_SOURCE_COMMIT='$(cat "$run_root/source_commit.txt")'
cd '$code_dir'
...
EOF
    ...
    current_commit=${SEMIGROUP_SOURCE_COMMIT:-$(cd "$code_dir" && git rev-parse HEAD)}
