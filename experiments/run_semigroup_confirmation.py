#!/usr/bin/env python3
"""Independent fixed-budget confirmation; see SCNET_CONFIRMATION_PROTOCOL.md."""
...
SEEDS = (3101, 3102, 3103, 3104, 3105, 3106)
LAGS = (.03, .07, .13, .23)
HORIZONS = (.6, 1.2, 2.4)

def matrix():
    return [dict(track='irregular', seed=seed, n=128, noise=noise, model=model)
            for seed in SEEDS for noise in (0., .01) for model in ('autonomous', 'query')]
...
def prepare(root,small=False):
    ...
    pools={}
    for split,seed in (('val',290901),('test',290902),('stress',290903)):
    ...
    try:
        ...
        for step in range(1,updates+1):
            ...
            if step==1 or step%100==0 or step==updates:
                ...
                if step in (2000,5000,10000,20000) or step==updates:
                    torch.save(model.state_dict(),out/f'fixed_{step:06d}.pt')
                    torch.save(torch.load(out/'best.pt',map_location='cpu'),out/f'best_{step:06d}.pt')
        ...
        budget_rows=[]
        for checkpoint in sorted(out.glob('best_*.pt')):
            budget=int(checkpoint.stem.split('_')[1])
            model.load_state_dict(torch.load(checkpoint,map_location=device))
            budget_rows.append(dict(budget=budget,checkpoint=checkpoint.name,
                rows=evaluate(model,data,cell['track'])))
        base.dump(out/'budget_evaluation.json',budget_rows)
        model.load_state_dict(torch.load(out/'best.pt',map_location=device))
        rows=evaluate(model,data,cell['track'])
...
def main():
    ...
    p.add_argument('--device',default='cuda');p.add_argument('--updates',type=int,default=20000)
