...
cp "$code_dir/docs/research/SCNET_CONFIRMATION_PROTOCOL.md" "$root/protocol.md"
...
cd '$code_dir'
'$python_bin' experiments/run_semigroup_confirmation.py prepare --root '$root/run'
'$python_bin' experiments/run_semigroup_confirmation.py run --root '$root/run' --cell 0 --updates 2 --smoke
'$python_bin' experiments/run_semigroup_confirmation.py run --root '$root/run' --cell 1 --updates 2 --smoke
printf 'passed\n' > '$root/smoke_passed'
INNER
...
[ -f '$root/smoke_passed' ]
cd '$code_dir'
'$python_bin' experiments/run_semigroup_confirmation.py run --root '$root/run' --cell "\$SLURM_ARRAY_TASK_ID" --updates 20000
INNER
smoke_id=$(sbatch --parsable --partition=xhhgnormal --gres=gpu:1 --ntasks=1 --cpus-per-task=4 --time=00:30:00 --output="$root/logs/smoke-%j.log" "$root/smoke.sh")
